...
@Injectable()
export class ContractInactivationCron {
  private readonly logger = new Logger(ContractInactivationCron.name);
  ...
  private async sendSummaryEmail(
    ...
    try {
      await this.emailService.sendHtmlEmail(
        ContractInactivationCron.NOTIFICATION_EMAIL,
        subject,
        htmlBody,
      );
      this.logger.log(
        `Summary email sent to ${ContractInactivationCron.NOTIFICATION_EMAIL} with ${contracts.length} inactivated contract(s).`,
      );
    } catch (error: unknown) {
      this.logger.error(
