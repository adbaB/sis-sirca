...
export class PaymentPdfCron {
  private readonly logger = new Logger(PaymentPdfCron.name);
  ...
  async generateAndSendPendingReceipts(): Promise<void> {
    ...
    const payments = await this.paymentService.findUnsetPayment();
    ...
    const isSend = await this.paymentService.markPaymentsAsSent(validPayments);
  ...
  private async calculateFinancialInfo(payment: Payment): Promise<{
    ...
    const numericRate = await this.exchangeRateService.resolvePaymentExchangeRate(payment);
