...
export function formatDateES(date: Date | string): string {
  ...
  return DateTime.fromJSDate(d).setZone('America/Caracas').toFormat('dd-MM-yyyy');
}
 ...
 */
export function getGeneratedAtTimestamp(): string {
 ...
 */
export async function loadLogoBase64(logger: Logger): Promise<string> {
  const attempts = [
    path.join(process.cwd(), 'src', 'assets', 'images', 'logo.png'),
    path.join(process.cwd(), 'dist', 'assets', 'images', 'logo.png'),
  ];
 ...
 */
export async function loadLogoImagePath(logger: Logger): Promise<string | null> {
  const attempts = [
    path.join(process.cwd(), 'src', 'assets', 'images', 'logo.png'),
    path.join(process.cwd(), 'dist', 'assets', 'images', 'logo.png'),
  ];
 ...
 */
export async function fetchAdvisorName(
 ...
 */
export async function finishWorkbook(workbook: ExcelJS.Workbook): Promise<Buffer> {
...
export function applyTitleRowStyle(cell: ExcelJS.Cell): void {
...
export function applySectionHeaderStyle(cell: ExcelJS.Cell): void {
...
export function applyTableHeaderStyle(cell: ExcelJS.Cell): void {
...
export function applyDataCellStyle(cell: ExcelJS.Cell): void {
...
export function applySubtotalCellStyle(cell: ExcelJS.Cell): void {
  cell.fill = {
    type: 'pattern',
    pattern: 'solid',
...
export function applyGrandTotalStyle(
  cell: ExcelJS.Cell,
...
export function thinBorder(color = BRAND_COLORS.borderColor): Partial<ExcelJS.Borders> {
