...
export class ContractCreationService {
  ...
  /**
   * Creates a basic empty contract with generated code and advisor relation.
   */
  ...
  async createFull(dto: CreateContractFullDto): Promise<Contract> {
    ...
    const savedContract = await this.contractsRepository.manager.transaction(async (manager) => {
      ...
      // Return contract with full relations loaded
    ...
    // 4. Invoice Generation (after transaction commits)
    try {
